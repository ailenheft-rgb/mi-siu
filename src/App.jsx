  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'} font-sans pb-12 transition-colors duration-300`}>
      <header className={`shadow-lg sticky top-0 z-20 border-b ${isDarkMode ? 'bg-slate-950 border-teal-900/50 text-white' : 'bg-gradient-to-r from-teal-600 to-emerald-600 text-white border-teal-700/50'}`}>
        <div className="max-w-6xl mx-auto px-4 py-4">
          ...
          <div className="flex gap-2 mt-6 overflow-x-auto pb-0 hide-scrollbar items-end">
            ...
            <TabButton isDarkMode={isDarkMode} active={activeTab === 'predictor'} onClick={() => setActiveTab('predictor')} icon={<Sparkles size={18} />} label="Mis Predicciones" />
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {activeTab === 'predictor' && <PredictorView isDarkMode={isDarkMode} user={user} subjects={INITIAL_SUBJECTS} subjectDetails={subjectDetails} />}
...
// ==========================================
// VIEW: MIS PREDICCIONES (NUEVO)
// ==========================================
function PredictorView({ isDarkMode, user, subjects, subjectDetails }) {
  ...
  // Cargamos las predicciones locales
  const [subjectPreds, setSubjectPreds] = useState(() => {
    try { return JSON.parse(localStorage.getItem('my_preds_' + user?.uid)) || {}; } 
    catch (e) { return {}; }
  });

  useEffect(() => {
    if (user) localStorage.setItem('my_preds_' + user.uid, JSON.stringify(subjectPreds));
  }, [subjectPreds, user]);

  const handlePredChange = (subId, field, val) => {
    setSubjectPreds(prev => ({
      ...prev,
      [subId]: { ...(prev[subId] || { p1: '', p2: '', final: '' }), [field]: val }
    }));
  };
  ...
  return (
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500">
      <div className="text-center mb-10">
        <Sparkles size={48} className={`mx-auto mb-4 ${isDarkMode ? 'text-teal-500' : 'text-teal-600'}`} />
        <h2 className={`text-3xl font-bold mb-2 ${textPrimary}`}>Mis Predicciones</h2>
        <p className={textSec}>Juega con tus notas. Ingresa cuánto crees que te sacarás en cada instancia para proyectar tu estado final en las materias que estás cursando.</p>
      </div>

      <div className={`mb-8 p-6 sm:p-8 rounded-2xl border shadow-sm ${bgCard}`}>
        {cursandoSubjects.length === 0 ? (
          <div className={`text-center py-8 ${textSec}`}>
            No estás cursando ninguna materia actualmente. Ve a "Mi Plan" y marca tus materias de este cuatrimestre como <span className="font-bold text-yellow-500">"Cursando"</span> para que aparezcan aquí.
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className={`border-b ${isDarkMode ? 'border-slate-700 text-slate-400' : 'border-slate-200 text-slate-500'}`}>
                  <th className="pb-3 pr-4 font-bold text-sm">Materia</th>
                  <th className="pb-3 px-2 font-bold text-sm text-center">Predicción P1</th>
                  <th className="pb-3 px-2 font-bold text-sm text-center">Predicción P2</th>
                  <th className="pb-3 px-2 font-bold text-sm text-center">Final (opcional)</th>
                  <th className="pb-3 pl-4 font-bold text-sm text-right">Proyección</th>
                </tr>
              </thead>
              <tbody>
                {cursandoSubjects.map(sub => {
                  const pred = subjectPreds[sub.id] || { p1: '', p2: '', final: '' };
                  const g1 = parseFloat(pred.p1);
                  const g2 = parseFloat(pred.p2);
                  const gFinal = parseFloat(pred.final);
                  
                  let resultText = <span className={`text-sm italic ${isDarkMode ? 'text-slate-600' : 'text-slate-400'}`}>Faltan notas...</span>;
                  
                  if (!isNaN(g1) && !isNaN(g2)) {
                    const avg = (g1 + g2) / 2;
                    if (avg >= 8) {
                      resultText = <span className="text-emerald-500 font-bold">🏆 Promoción ({avg.toFixed(1)})</span>;
                    } else if (avg >= 6) {
                      if (!isNaN(gFinal)) {
                        if (gFinal >= 6) resultText = <span className="text-green-500 font-bold">🟢 Aprobada ({gFinal})</span>;
                        else resultText = <span className="text-red-500 font-bold">🔴 Final Desaprobado</span>;
                      } else {
                        resultText = <span className="text-blue-500 font-bold">📝 Regular ({avg.toFixed(1)})</span>;
                      }
                    } else {
                      resultText = <span className="text-red-500 font-bold">🔴 Libre ({avg.toFixed(1)})</span>;
                    }
                  } else if (!isNaN(g1)) {
                     resultText = <span className={`text-sm italic ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Esperando P2...</span>;
                  }

                  return (
                    <tr key={sub.id} className={`border-b last:border-0 ${isDarkMode ? 'border-slate-800/50 hover:bg-slate-800/30' : 'border-slate-100 hover:bg-slate-50'} transition-colors`}>
                      <td className={`py-4 pr-4 font-semibold text-sm ${textPrimary}`}>{sub.name}</td>
                      <td className="py-4 px-2 text-center">
                        <input type="number" min="1" max="10" placeholder="-" className={`w-16 p-2 rounded-lg border outline-none focus:ring-2 focus:ring-teal-500 text-center font-bold ${inputBg}`} value={pred.p1} onChange={e => handlePredChange(sub.id, 'p1', e.target.value)} />
                      </td>
                      <td className="py-4 px-2 text-center">
                        <input type="number" min="1" max="10" placeholder="-" className={`w-16 p-2 rounded-lg border outline-none focus:ring-2 focus:ring-teal-500 text-center font-bold ${inputBg}`} value={pred.p2} onChange={e => handlePredChange(sub.id, 'p2', e.target.value)} />
                      </td>
                      <td className="py-4 px-2 text-center">
                        <input type="number" min="1" max="10" placeholder="-" className={`w-16 p-2 rounded-lg border outline-none focus:ring-2 focus:ring-purple-500 text-center font-bold ${inputBg}`} value={pred.final} onChange={e => handlePredChange(sub.id, 'final', e.target.value)} title="Solo si quedas regular" />
                      </td>
