...
function PredictorView({ isDarkMode, user, subjects, subjectDetails }) {
  ...
  // --- NUEVO: PLANILLA POR MATERIA ---
  const cursandoSubjects = useMemo(() => {
    return subjects.filter(s => subjectDetails[s.id]?.status === 'cursando');
  }, [subjects, subjectDetails]);

  // Cargamos las predicciones guardadas de forma local para no afectar Firebase
  const [subjectPreds, setSubjectPreds] = useState(() => {
    try { return JSON.parse(localStorage.getItem('preds_' + user?.uid)) || {}; } 
    catch (e) { return {}; }
  });

  // Guardamos los cambios automáticamente
  useEffect(() => {
    if (user) localStorage.setItem('preds_' + user.uid, JSON.stringify(subjectPreds));
  }, [subjectPreds, user]);

  const handlePredChange = (subId, field, val) => {
    setSubjectPreds(prev => ({
      ...prev,
      [subId]: { ...(prev[subId] || { p1: '', p2: '', meta: 8 }), [field]: val }
    }));
  };
  ...
  return (
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500">
      ...
      {/* PLANILLA POR MATERIAS (NUEVO) */}
      <div className={`mb-8 p-6 sm:p-8 rounded-2xl border shadow-sm ${bgCard}`}>
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200/20">
          <div className={`p-2 rounded-lg ${isDarkMode ? 'bg-blue-900/50 text-blue-400' : 'bg-blue-100 text-blue-700'}`}><BookOpen size={24} /></div>
          <h3 className={`text-xl font-bold ${textPrimary}`}>Planilla de Predicción por Materia</h3>
        </div>
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className={`border-b ${isDarkMode ? 'border-slate-700 text-slate-400' : 'border-slate-200 text-slate-500'}`}>
                  ...
                  <th className="pb-3 px-2 font-bold text-sm text-center">Parcial 1</th>
                  <th className="pb-3 px-2 font-bold text-sm text-center">Parcial 2</th>
                  <th className="pb-3 px-2 font-bold text-sm text-center">Meta</th>
                  <th className="pb-3 pl-4 font-bold text-sm text-right">Nota Necesaria</th>
                </tr>
              </thead>
              <tbody>
                {cursandoSubjects.map(sub => {
                  const pred = subjectPreds[sub.id] || { p1: '', p2: '', meta: 8 };
                  const g1 = parseFloat(pred.p1);
                  const g2 = parseFloat(pred.p2);
                  const validG = [g1, g2].filter(g => !isNaN(g));
                  const sum = validG.reduce((a,b) => a+b, 0);
                  const meta = parseFloat(pred.meta) || 8;
                  
                  let needed = null;
                  if (validG.length < 2) {
                     needed = (meta * (validG.length + 1)) - sum;
                  }

                  return (
                    <tr key={sub.id} className={`border-b last:border-0 ${isDarkMode ? 'border-slate-800/50' : 'border-slate-100'}`}>
                      <td className={`py-4 pr-4 font-semibold text-sm ${textPrimary}`}>{sub.name}</td>
                      <td className="py-4 px-2 text-center">
                        <input type="number" min="1" max="10" placeholder="-" className={`w-16 p-2 rounded-lg border outline-none focus:ring-2 focus:ring-blue-500 text-center font-bold ${inputBg}`} value={pred.p1} onChange={e => handlePredChange(sub.id, 'p1', e.target.value)} />
                      </td>
                      <td className="py-4 px-2 text-center">
                        <input type="number" min="1" max="10" placeholder="-" className={`w-16 p-2 rounded-lg border outline-none focus:ring-2 focus:ring-blue-500 text-center font-bold ${inputBg}`} value={pred.p2} onChange={e => handlePredChange(sub.id, 'p2', e.target.value)} />
                      </td>
                      <td className="py-4 px-2 text-center">
                        <select className={`w-20 p-2 rounded-lg border outline-none focus:ring-2 focus:ring-blue-500 font-bold ${inputBg}`} value={pred.meta} onChange={e => handlePredChange(sub.id, 'meta', e.target.value)}>
                      ...
                      <td className="py-4 pl-4 text-right">
                        {validG.length === 0 ? (
                          <span className={`text-xs font-medium ${textSec}`}>Ingresa Parcial 1</span>
                        ) : validG.length === 2 ? (
                          <span className={`text-sm font-bold ${isDarkMode ? 'text-green-400' : 'text-green-600'}`}>Definitiva: {(sum/2).toFixed(1)}</span>
                        ) : (
                          needed > 10 ? (
                             <span className={`text-xs font-bold px-2 py-1 rounded ${isDarkMode ? 'bg-red-900/50 text-red-400' : 'bg-red-100 text-red-700'}`}>Imposible ({needed.toFixed(1)})</span>
                          ) : needed <= 1 ? (
                             <span className={`text-xs font-bold px-2 py-1 rounded ${isDarkMode ? 'bg-green-900/50 text-green-400' : 'bg-green-100 text-green-700'}`}>Aprobado (1)</span>
                          ) : (
                             <span className={`text-lg font-black ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>{needed.toFixed(1)}</span>
                          )
                        )}
                      </td>
                    </tr>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className={`p-6 sm:p-8 rounded-2xl border shadow-sm ${bgCard}`}>
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200/20">
            <div className={`p-2 rounded-lg ${isDarkMode ? 'bg-teal-900/50 text-teal-400' : 'bg-teal-100 text-teal-700'}`}><Target size={24} /></div>
            <h3 className={`text-xl font-bold ${textPrimary}`}>Calculadora Rápida</h3>
          </div>
